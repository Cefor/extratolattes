...
pontos_tec <- function(item, param = NULL){
  ...
  if(item == 1){
    ...
    # 4 - apresentacao de trabalho (palestrante, conferencista, participa??o em mesa redonda)
    list("T2", 1) # DISCENTE
  } else if(item == 5){
    ...
    if(is.null(param)) {
      ...
      list("T2o", 1) # Outro tipo de produto/servico tecnico especializado
    }
  } else if(item == 8){
    # 8 - trabalho tecnico
    if(param[1] == "PARECER"){
    ...
    if(param == "registro de software"){ 
      list("T4", 2)
    } else list("T2", 1) # outro tipo de produto/servico tecnico especializado
  } else if(item == 10){
