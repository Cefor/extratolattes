...
df2 <- rbind(df2, c(0, colSums(df2[ , 2:ncol(df2)]))) # inclui linha para o somatório de artigos por estrato
df2 <- rbind(df2, c(0, colSums(df2[ , 2:ncol(df2)]) * pontos_artigo(estratos)) / 2) # inclui linha com a pontuação

df2 <- cbind(df2, rowSums(df2[ , 2:ncol(df2)])) # inclui coluna para o somatório de artigos por nome
...
df2 <- rbind(df2, c(0, colSums(df2[ , 2:ncol(df2)]))) # inclui linha para o somatório de livros por tipo

df2 <- cbind(df2, rowSums(df2[ , 2:ncol(df2)])) # inclui coluna para o somatório de livros por nome
names(df2)[ncol(df2)] <- "TOTAL"
...
indicadores <- rbind(indicadores,
                     data.frame(indicador = "Indicador de Produção de Livros", 
                            valor = (sum(df2$LIVRO_PUBLICADO) + sum(df2$LIVRO_ORGANIZADO_OU_EDICAO))/n_docentes/2)
                     )
...
df2 <- rbind(df2, c(0, colSums(df2[ , 2:ncol(df2)]))) # inclui linha para o somatório de prod. ténica por estrato

df2 <- rbind(df2, round(c(0, colSums(df2[ , 2:ncol(df2)]) * pontos_tec(0, estratos) / 2), 0) ) # inclui linha com a pontuação

df2 <- cbind(df2, rowSums(df2[ , 2:ncol(df2)])) # inclui coluna para o somatório de p. técnica por nome
names(df2)[ncol(df2)] <- "TOTAL"
...
# Docentes com orientação como principal
df <- producao_orientacoes %>% 
              select(id, tipo, nivel, situacao) %>% 
              filter(tipo %in% c('orientador principal'), 
